...
describe('accounting invariants', () => {
  it('reconciles ledger cash to all explicit movements', () => {
    ...
    assert.equal(reconcileLedgerCash(100000, ledger, 99985).length, 0);
  });
  ...
  it('catches corrupted equity', () => {
    ...
    assert.throws(() => assertEquityPoint(point, 1000, 0, { timestamp: 1735689600000, symbol: 'BTCUSDT' }), /Equity point/);
  });
  ...
  it('rejects impossible position quantities', () => {
    ...
    assert.throws(() => assertPositionQuantity(position, { symbol: 'BTCUSDT', timestamp: 1 }), /non-negative/);
  });
  ...
  it('requires funding event and ledger count to reconcile', () => {
    ...
    assert.equal(auditFunding([event], ledger).length, 1);
  });
  ...
  it('requires liquidation to have an execution', () => {
    ...
    assert.equal(auditLiquidations([event], [], ledger, []).length, 1);
  });
  ...
  it('reconciles execution fees to fee ledger movements', () => {
    ...
    assert.equal(auditExecutionsHaveLedgerCashEffects([execution], ledger).length, 0);
  });
});
